...
```mermaid
flowchart LR
    YT["1 YouTube long-form<br/>(the weekly seed)"] --> SH["2–3 Shorts<br/>(claim-card cuts)"]
    SH --> TT["1–2 TikToks<br/>(re-edited natively,<br/>no watermarks)"]
    SH --> RL["1–2 Reels<br/>(re-exported)"]
    YT --> BL["1 blog post<br/>(script expanded,<br/>full citations)"]
    BL --> NL["1 newsletter section<br/>(The External Brain)"]
    BL --> CA["1 carousel<br/>(8–10 slides + Sources slide)"]
    BL --> PN["3–5 pins<br/>(+ honest fresh-pin variants)"]
    CA --> PN
```
